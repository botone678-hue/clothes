...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    async function initAuth() {
      ...
      // Check cached local user session
      try {
        ...
        if (stored) {
          ...
          // Default to a guest customer profile for instantaneous exploration
  ...
  const signIn = async (email: string, password?: string, role?: UserRole): Promise<{ success: boolean; error?: string }> => {
    ...
    if (isSupabaseConfigured && password) {
      try {
        ...
        console.warn('Supabase signin error, checking local profiles:', err);
      }
    }
    ...
    if (!matched) {
      // If role specified or standard sign in
      matched = await db.saveProfile({
        full_name: email.split('@')[0].replace('.', ' '),
        email,
        phone: '0741775878',
        role: role || (email.includes('admin') ? 'admin' : email.includes('driver') ? 'driver' : 'customer'),
        status: 'active',
      });
    }
  ...
  const signUp = async (
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        console.warn('Supabase signup fallback to database profile:', err);
      }
    }
  ...
  const signOut = async () => {
    ...
    localStorage.setItem(AUTH_USER_KEY, JSON.stringify(guestCustomer));
  };
  ...
  const resetPassword = async (email: string): Promise<{ success: boolean; message?: string; error?: string }> => {
    ...
    return {
      success: true,
      message: `Password reset link generated for ${email}. (In production with Supabase, an email with a secure token is dispatched).`,
    };
  };
  ...
  const switchUserRole = (targetRole: UserRole) => {
    if (targetRole === 'admin') {
      ...
      localStorage.setItem(AUTH_USER_KEY, JSON.stringify(INITIAL_ADMIN));
    } else if (targetRole === 'driver') {
      const driver = INITIAL_DRIVERS[0];
      setUser(driver);
      localStorage.setItem(AUTH_USER_KEY, JSON.stringify(driver));
    } else {
      const cust: Profile = {
        id: 'cust-demo-1',
        full_name: 'Wanjiku Mwangi',
      ...
      setUser(cust);
      localStorage.setItem(AUTH_USER_KEY, JSON.stringify(cust));
    }
  };
