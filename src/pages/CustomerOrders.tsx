...
import { Package, Clock, MapPin, Calendar, ArrowRight, Eye, RefreshCw, Sparkles } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={loadOrders}
            className="p-2.5 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl text-slate-600 transition cursor-pointer"
          ...
          <button
            ...
            className="px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold rounded-xl shadow-md transition cursor-pointer flex items-center gap-1.5"
          >
            <Sparkles className="w-4 h-4" />
            <span>Book New Laundry</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        {[
          ...
          { id: 'processing', label: 'In Washing/Spa' },
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 shadow-sm space-y-4">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            You haven't placed any laundry orders yet. Experience Eldoret's premier garment care with doorstep pickup!
          </p>
          <button
            onClick={() => setCurrentTab('book')}
            className="px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold rounded-xl shadow-md transition cursor-pointer"
          >
            Book My First Laundry Pickup
          </button>
        </div>
      ) : (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-slate-600">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    <strong>Schedule:</strong> {order.pickup_date} ({order.pickup_time})
                  </span>
                </div>
                <div className="flex items-center gap-2">
              ...
              {order.items && order.items.length > 0 && (
                <div className="p-3 bg-slate-50 rounded-2xl flex flex-wrap gap-2 text-[11px] text-slate-700">
                  {order.items.map((it) => (
                    <span
                      key={it.id}
                      className="px-2.5 py-1 bg-white border border-slate-200 rounded-lg font-medium"
                    >
                      {it.service_name} (x{it.quantity})
                    </span>
                  ))}
                </div>
              )}
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2 bg-sky-50 hover:bg-sky-100 text-sky-700 font-bold text-xs rounded-xl flex items-center gap-1.5 transition cursor-pointer"
                >
